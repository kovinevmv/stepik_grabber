...
class StepikGrabber:
    ...
    def grab_answers(self):
        ...
        for index, section in enumerate(sections, start=1):
            units[section]['title'] = '{}. {}'.format(index,
                                                      units[section]['title'])

        self.main_json = units

        for index_section, (section_id, unit_data) in enumerate(units.items(),
                                                                start=1):
            lessons = self.stepik_api.get_lessons_from_units(
                unit_data['units'])
            ...
            for lesson_id, lesson_info in steps.items():
                index_of_l = self.main_json[section_id]['units'].index(
                    lesson_id)
                lesson_info['title'] = '{}. {}'.format(index_of_l + 1,
                                                       lesson_info['title'])
                lesson_info.update({'num': index_of_l + 1})
                self.main_json[section_id]['units'][index_of_l] = {
            ...
            for lesson_id, lesson_data in steps.items():
                ...
                self.main_json[section_id]['units'][index_of_l][lesson_id][
                    'steps'] = []
                for index_step, step in enumerate(steps_list, start=1):
                    print(
                        f'Current step: {step}. Section: {index_section}/{len(units)}'
                    )
                    attempts = self.stepik_api.get_attempts_of_step(step)
                    if attempts is not None:
                        sub = self.stepik_api.get_submissions_of_step(step)
                        answer = self.stepik_api.convert_solution(
                            attempts, sub)
                        self.main_json[section_id]['units'][index_of_l][
                            lesson_id]['steps'].append({
                                'num':
                                '{}-{}-{}'.format(index_section,
                                                  index_of_l + 1, index_step),
                                'answer':
                                answer
                            })
